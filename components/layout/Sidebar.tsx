...
// Top-level navigation. `Plan` is the primary verb — it lands on the
// canvas where every planning action (pick people, pick a date, share a
// link) is an in-canvas affordance rather than its own destination. The
// other items are views onto existing state.
const navItems = [
  { href: "/",         label: "Home",     icon: LayoutDashboard, exact: true },
  { href: "/plan",     label: "Plan",     icon: Plus,            primary: true },
    ...
    <TooltipProvider delayDuration={300}>
      <aside className={cn(
        ...
        <nav className="flex-1 p-2 space-y-1 overflow-y-auto">
          {navItems.map(({ href, label, icon: Icon, exact, primary }) => {
            const isActive = exact ? pathname === href : pathname.startsWith(href);
            // `Plan` gets a subtle accent when idle so it reads as the
            // primary verb even without being the active route. When
            // active it follows the same treatment as any other item.
            return (
              <Link
                key={href}
                href={href}
